...
function createPageLink(pageNum, linkText) {
    if (type === "page") {
        ...
        let baseUrl = `${window.location.origin}/search?${searchParam}&updated-max=${encodeURIComponent(lastPostDate || new Date().toISOString())}&max-results=${itemsPerPage}&start=${startIndex}&by-date=false`;
        baseUrl = addMobileParam(baseUrl);

        let url = `${baseUrl}#PageNo=${pageNum}`;
...
function redirectlabel(pageNum) {
    if (pageNum === 1) {
        let url = `${window.location.origin}/search/label/${lblname1}?max-results=${itemsPerPage}`;
        url = addMobileParam(url);
        url += "#PageNo=1";
        location.href = url;
        return;
    }
...
function finddatepost(data) {
    ...
    let redirectUrl = type === "page"
        ? `/search?updated-max=${encodedDate}&max-results=${itemsPerPage}`
        : `/search/label/${lblname1}?updated-max=${encodedDate}&max-results=${itemsPerPage}`;

    redirectUrl = addMobileParam(redirectUrl);
    redirectUrl += `#PageNo=${nopage}`;
